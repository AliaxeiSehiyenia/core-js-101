...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let result;
  if (a < b) result = `${isStartIncluded ? '[' : '('}${a}, ${b}${isEndIncluded ? ']' : ')'}`;
  else result = `${isStartIncluded ? '[' : '('}${b}, ${a}${isEndIncluded ? ']' : ')'}`;
  return result;
}
...
function isCreditCardNumber(ccn) {
  let result = false;
  if (String(ccn).length % 2 === 1) {
    const sum = String(ccn).split('').map((elem, index) => {
      if (index % 2 === 1) {
        return (elem * 2) >= 10 ? String((elem * 2) - 9) : String((elem * 2));
      }
      return elem;
...
function getDigitalRoot(num) {
  ...
  while (n.toString().length > 1) {
    n = String(n).split('').reduce((acc, cur) => +acc + +cur);
  }
  return n;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i <= str.length - 1; i += 1) {
    ...
    if (bracketClose.indexOf(str[i]) === bracketOpen.indexOf(stack[stack.length - 1])) {
      stack.pop();
    }
